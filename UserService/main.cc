...
auto CreateInjector(const std::shared_ptr<boost::asio::io_context> &ioc_ptr, const RedisConfig &redis_config,
                    const DbPoolConfig &db_pool_config) {
    /*
...
int main() {
    ...
    try {
        ...
        const RedisConfig redis_config = {"grpc-dev-redis.bbqzfi.ng.0001.apse1.cache.amazonaws.com", "6379"};
        const DbPoolConfig db_pool_config = {
            "postgresql://lxy:12345678@grpc-dev-database.cvosiiw6iypg.ap-southeast-1.rds.amazonaws.com:5432/postgres", 4
        };
        const auto injector = CreateInjector(ioc_ptr, redis_config, db_pool_config);
        ...
        // 这里的协程需要同步等待
        std::future<void> init_future = boost::asio::co_spawn(*ioc_ptr, [&injector]() -> boost::asio::awaitable<void> {
            try {
                // 1. Redis 初始化
                SPDLOG_INFO("Starting RedisClient...");
                auto &redis_client = injector.create<RedisClient &>();
                co_await redis_client.Init();
                SPDLOG_INFO("RedisClient started.");

                // 2. DB 初始化
                SPDLOG_INFO("Starting Postgresql Connection Pool...");
                auto &postgresql_conn_pool = injector.create<AsyncConnectionPool &>();
                co_await postgresql_conn_pool.Init();
                SPDLOG_INFO("Postgresql Connection Pool started.");
            } catch (const std::exception &e) {
                SPDLOG_CRITICAL("Startup failed: {}", e.what());
                std::exit(EXIT_FAILURE); // 初始化失败直接退出
            }
